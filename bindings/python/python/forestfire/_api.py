...
from . import _core
from ._categorical import CategoricalPreprocessor, fit_categorical_preprocessor
...
class Model:
    def __init__(
        self,
        inner: _core.Model,
        preprocessor: CategoricalPreprocessor | None = None,
        *,
    ...
    def predict(self, x: Any) -> Any:
        ...
        return self._inner.predict(self._preprocessor.transform(x))

    def predict_proba(self, x: Any) -> Any:
        if self._preprocessor is None:
            return self._inner.predict_proba(x)
        return self._inner.predict_proba(self._preprocessor.transform(x))
    ...
    def to_ir_json(self, pretty: bool = False) -> str:
        if self._preprocessor is not None:
            raise ValueError(
                "IR export is not supported for models trained with categorical preprocessing."
            )
    ...
    def serialize(self, pretty: bool = False) -> str:
        if self._preprocessor is not None:
            raise ValueError(
                "Serialization is not supported for models trained with categorical preprocessing."
            )
...
class OptimizedModel:
    ...
    def serialize_compiled(self) -> bytes:
        if self._preprocessor is not None:
            raise ValueError(
                "Compiled serialization is not supported for models trained with categorical preprocessing."
            )
...
def train(
    ...
    **kwargs: Any,
    ...
    preprocessor, encoded_x = fit_categorical_preprocessor(
        x,
        y,
    ...
    return Model(
        _core.train(encoded_x, y, **kwargs),
        preprocessor,
