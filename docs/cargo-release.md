...
- `forestfire-core` cannot be packaged against crates.io until
  `forestfire-data` has already been published and indexed at the current
  version, because the packaged crate resolves `forestfire-data` from crates.io
  rather than from the workspace path dependency.
